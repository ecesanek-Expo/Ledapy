...
def getResult(
    raw_vector, result_type, sampling_rate, downsample=1, optimisation=0, pipeout=None
):
    """
    ...
    if pipeout is not None:
        ...
        return leda2  # result # Modified by EC to return complete leda2 object
...
def import_data(raw_data, srate, downsample=1):
    ...
    if downsample > 1:
        (time_data, conductance_data) = utils.downsamp(
            time_data, conductance_data, downsample, "mean"
        )
    ...
    (leda2.data.conductance_smoothData, leda2.data.conductance_smoothData_win) = (
        utils.smooth_adapt(conductance_data, srate, 0.00001)
    )
